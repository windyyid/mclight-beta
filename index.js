...
const [target] = args
  if (target === '0.0.0.0') {
  console.log(`oi napa elu malah ddos server sendiri kocak`)
	sigma()
  } else {
    try {
...
ovh https://example.com 60 10 100`);
  ...
  if (isNaN(time) || isNaN(threads) || isNaN(rps)) {
    console.log('Time, threads and RPS must be numbers');
    sigma();
    return;
  }
  ...
  if (!fs.existsSync(proxyPath)) {
    console.log('Proxy file not found. Please run update-proxy first.');
    sigma();
    return;
  }
...
Type "exit" To Stop Chat`);
    ...
    console.log(`Gada Command Begituan Anying - ${command}`);
    sigma();
  }
});
}
...
udp-bypass 123.456.78.910 53 300`);
  ...
  if (!fs.existsSync(proxyPath)) {
    console.log('SOCKS5 proxy file not found. Please run update-proxy first.');
    sigma();
    return;
  }
